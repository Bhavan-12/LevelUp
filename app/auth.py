...
def get_current_user(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security), conn = Depends(get_db)):
    """FastAPI dependency to authenticate requests using JWT Bearer headers."""
    if not credentials:
        raise HTTPException(
